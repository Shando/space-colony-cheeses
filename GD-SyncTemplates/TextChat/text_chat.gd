...
func enter_channel(channel : int):
	if _listening_channels.has(channel): 
		return
...
func _send_message():
	...
	var res = TTS.checkProfanity(text)
	...
	if not res:
		...
		for client_id in GDSync.get_all_clients():
			...
			if not GDSync.get_player_data(client_id, "TextChannels", []).has(_current_typing_channel): 
				continue

			GDSync.call_func_on(client_id, _receive_message, [text, _current_typing_channel, GDSync.get_client_id()])
	else:
		text = "[PROFANITY DETECTED] Cannot send message as it appears to contain Profanity."
		_receive_message(text, _current_typing_channel, GDSync.get_client_id())
...
func _receive_message(text : String, channel : int, from : int, from_save : bool = false):
	...
	if show_channel_id or show_usernames: 
		message += " "
