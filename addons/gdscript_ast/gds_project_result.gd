...
var _target_file_edges: Dictionary = {}  # String(target_file) → Array[GDSCrossFileEdge]（入向边索引）
...
func add_edge(p_edge) -> void:
	...
	# 维护 target_file → 入向边索引
	if not _target_file_edges.has(p_edge.target_file):
		_target_file_edges[p_edge.target_file] = []
	_target_file_edges[p_edge.target_file].append(p_edge)


# 查询: 指向 p_target_file 的所有入向跨文件边
func get_incoming_cross_edges(p_target_file: String) -> Array:
	return _target_file_edges.get(p_target_file, [])
