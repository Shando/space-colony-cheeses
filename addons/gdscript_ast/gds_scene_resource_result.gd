...
func get_connections_for_node(p_node_path: String) -> Array:
	...
	if root_nodes.is_empty():
		return result
	var root_name: String = root_nodes[0].name
	# 把路径中的根节点名或 "." 统一归一化为根节点名，便于和 tscn 中相对路径匹配
	var normalized_input: String = root_name if p_node_path in [".", root_name] else p_node_path
	for conn in signal_connections:
		var c: SignalConnectionData = conn
		var from_node: String = root_name if c.from_node == "." else c.from_node
		var to_node: String = root_name if c.to_node == "." else c.to_node
		if from_node == normalized_input or to_node == normalized_input:
			result.append(c)
	return result
