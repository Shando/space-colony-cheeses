...
func _expand_one(p_node, p_result, p_visited: Dictionary, p_instance_path: String) -> void:
	...
	# 同时保留 instance 节点上已有的覆盖子节点（parent="." 的节点在 _build_node_tree 时已挂上）
	var override_children = p_node.children.duplicate()
	p_node.children = sub_root.children
	for child in override_children:
		p_node.children.append(child)
		# 更新覆盖子节点的 parent_path 与 flat key
		var old_key: String = child.name if child.parent_path in [".", ""] else child.parent_path + "/" + child.name
		if p_result.nodes_flat.has(old_key):
			p_result.nodes_flat.erase(old_key)
		child.parent_path = p_instance_path
		p_result.nodes_flat[p_instance_path + "/" + child.name] = child

	# 合并子场景 flat 节点（含递归展开子场景内的 instance）
	_merge_sub_flat(p_node, p_result, sub_result, p_instance_path, p_visited)

	# 合并子场景 signal_connections，路径按 instance 前缀重写
	_merge_sub_connections(p_result, sub_result, p_instance_path)
...
func _merge_sub_connections(p_result: GDSSceneResourceResult, p_sub_result: GDSSceneResourceResult, p_instance_path: String) -> void:
	# 子场景根节点名（连接路径中的根）
	var sub_root_name: String = ""
	if not p_sub_result.root_nodes.is_empty():
		sub_root_name = p_sub_result.root_nodes[0].name

	for conn in p_sub_result.signal_connections:
		var c: GDSSceneResourceResult.SignalConnectionData = conn
		var new_conn = GDSSceneResourceResult.SignalConnectionData.new()
		new_conn.signal_name = c.signal_name
		new_conn.from_node = _rewrite_connection_path(c.from_node, p_instance_path, sub_root_name)
		new_conn.to_node = _rewrite_connection_path(c.to_node, p_instance_path, sub_root_name)
		new_conn.method = c.method
		new_conn.flags = c.flags
		new_conn.binds = c.binds.duplicate()
		new_conn.unbinds = c.unbinds
		p_result.signal_connections.append(new_conn)


func _rewrite_connection_path(p_path: String, p_instance_path: String, p_sub_root_name: String) -> String:
	# 子场景根路径 "." 或根节点名 → instance 节点路径
	if p_path == "." or p_path == p_sub_root_name:
		return p_instance_path
	# 以根节点名开头的路径 → 替换为 instance 路径前缀
	if p_sub_root_name != "" and p_path.begins_with(p_sub_root_name + "/"):
		return p_instance_path + p_path.substr(p_sub_root_name.length())
	# 其余相对路径直接挂在 instance 节点下
	return p_instance_path + "/" + p_path
