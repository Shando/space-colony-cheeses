...
func _try_resolve_cross_call(p_source_file: String, p_edge, p_obj_type: String, p_symbol: String, p_kind: int, p_project: GDScriptProjectResult) -> void:
	...
	if p_kind == GDSCrossFileEdge.Kind.SIGNAL_CONNECT:
		xedge.callback = _extract_signal_callback(p_edge)
	p_project.add_edge(xedge)


# 从 SIGNAL_CONNECT 调用边提取回调方法名
# obj.signal.connect(cb) → arguments[0] 是 cb（IdentifierNode）
# obj.connect("sig", cb) → arguments[0] 是信号字面量，返回 ""（此形态 callee 即回调，不产生有效跨文件边）
func _extract_signal_callback(p_edge) -> String:
	if p_edge.arguments == null or p_edge.arguments.is_empty():
		return ""
	var cb = p_edge.arguments[0]
